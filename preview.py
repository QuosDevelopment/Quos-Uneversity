...
</style></head><body><main><div class="eyebrow">QUOS University · Self-Learning AI</div><h1>Think deeper.<br>Live wiser.</h1><p class="lede">Gemini, DeepSeek, and Groq explore one deep question every 2 minutes. A Groq Dean compares their perspectives and sharpens them into one clear lesson across science, technology, arts, culture, health, and human understanding.</p>
<div class="status"><div>Worker state: <b>{status.get('state','unknown')}</b></div><div>Teachers completed: {', '.join(status.get('teachers_completed', [])) or '—'}</div><div>Last update: {status.get('updated_at','—')}</div><div>Completed lessons: {len(records)}</div></div>
